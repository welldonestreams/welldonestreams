window.Games.blackjack = function() {
  ...
  let committed = 0; // chips wagered this round (checked against server balance)
  ...
  btns.deal.onclick = async () => {
    ...
    await window.GameAPI.getBalance(); // fresh number before committing
    if (currentBet > bal()) { ui.msg.textContent = 'Insufficient chips!'; return; }
  ...
  function checkHandLogic() {
    ...
    setButtons('play');
    btns.dbl.disabled = hand.cards.length > 2 || (committed + hand.bet) > bal();
  ...
  btns.dbl.onclick = () => {
    ...
    if (committed + hand.bet > bal()) { ui.msg.textContent = 'Not enough chips to double!'; return; }
    committed += hand.bet;
  ...
  btns.splt.onclick = () => {
    ...
    if (committed + h.bet > bal()) { ui.msg.textContent = 'Not enough chips to split!'; return; }
    committed += h.bet;
  ...
  async function finalizeGame() {
    ...
    playerHands.forEach((hand, i) => {
      ...
      // equal scores = push, no change

      if (handWon) { const el = document.getElementById(`p-hand-${i}`); if (el) el.classList.add('win-anim'); }
    });

    const resultMsg = netWinLoss > 0 ? `You WON 🪙${CasinoShared.formatAmt(netWinLoss)}!`
      : (netWinLoss < 0 ? `You lost 🪙${CasinoShared.formatAmt(Math.abs(netWinLoss))}.` : `Push. Bets returned.`);

    try {
      // Settle the whole round with the server in one call.
      ...
      setTimeout(() => { setButtons('bet'); currentBet = 0; committed = 0; updateBetVisuals(); }, 3000);
    }
  }
};